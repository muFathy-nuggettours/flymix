	...
	if ($penalties && (!$system_settings["hide_cancel_fees"] || !$system_settings["hide_change_fees"])) {
		...
		if (!$system_settings["hide_cancel_fees"]) {
			...
			foreach ($penalties as $key => $value) {
				...
				switch ($target["amount"]) {
					case "100%":
						$amount = "<b style='color:red'>غير قابلة للإلغاء</b>";
						break;
					case "0%":
						$amount = "<b style='color:green'>قابلة للإلغاء</b>";
						break;
					default:
						$amount = naRes($target["amount"], "<b style='color:red'>" . $target["amount"] . "</b>");
				}
		...
		if (!$system_settings["hide_change_fees"]) {
			...
			foreach ($penalties as $key => $value) {
				...
				switch ($target["amount"]) {
					case "100%":
						$amount = "<b style='color:red'>غير قابلة للتغيير</b>";
						break;
					case "0%":
						$amount = "<b style='color:green'>قابلة للتغيير</b>";
						break;
					default:
						$amount = naRes($target["amount"], "<b style='color:red'>" . $target["amount"] . "</b>");
				}
...
function createKeywords($data)
{
	$keywords = array();
	if ($data["iata"]) {
		array_push($keywords, strtolower($data["iata"]));
	}
	if ($data["code"]) {
		array_push($keywords, strtolower($data["code"]));
	}
	if ($data["ar_name"]) {
		array_push($keywords, normalizeString($data["ar_name"]));
	}
	if ($data["en_name"]) {
		array_push($keywords, strtolower(normalizeString($data["en_name"])));
	}
	if ($data["ar_short_name"]) {
		array_push($keywords, normalizeString($data["ar_short_name"]));
	}
	if ($data["en_short_name"]) {
		array_push($keywords, strtolower(normalizeString($data["en_short_name"])));
	}
	if ($data["alias"]) {
		array_push($keywords, strtolower(normalizeString($data["alias"])));
	}
...
function createSlugs($data)
{
	$slugs = array();
	if ($data["iata"]) {
		array_push($slugs, createCanonical($data["iata"]));
	}
	if ($data["code"]) {
		array_push($slugs, createCanonical($data["code"]));
	}
	if ($data["ar_name"]) {
		array_push($slugs, createCanonical($data["ar_name"]));
	}
	if ($data["en_name"]) {
		array_push($slugs, createCanonical($data["en_name"]));
	}
	if ($data["ar_short_name"]) {
		array_push($slugs, createCanonical($data["ar_short_name"]));
	}
	if ($data["en_short_name"]) {
		array_push($slugs, createCanonical($data["en_short_name"]));
	}
...
function getDuration($seconds, $separator = ", ", $show_days = true, $show_hours = true, $show_minutes = true, $show_seconds = false)
{
	...
	$value = array();
	if ($days && $show_days) {
		array_push($value, $days . " " . $language["day"]);
	}
	if ($hours && $show_hours) {
		array_push($value, $hours . " " . $language["hour"]);
	}
	if ($minutes && $show_minutes) {
		array_push($value, $minutes . " " . $language["minute"]);
	}
	if ($seconds && $show_seconds) {
		array_push($value, $seconds . " " . $language["second"]);
	}
...
function builtPageBlock($type, $entry)
{
	...
	switch ($type) {
		...
		case "en_website_pages":
		case "ar_website_pages":
			$block["title"] = $entry["title"];
			$block["description"] = $entry["description"];
...
//Save E-Mail
function saveMail($response)
{ //$response -> array($emails, $subject, $message, $response_status, $response_message)
	global $logged_user;
...
{ //$response -> array($mobile_numbers, $message, $response_status, $response_message)
	global $logged_user;
...
{ //$response -> array($users, $title, $message, $response_status, $response_message)
	global $logged_user;
